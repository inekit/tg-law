...
.addStep({variable: "count", cb: async (ctx)=>{
  ...
  const orderId = ctx.wizard.state.orderId = (await connection.query("insert into orders (count, sum, customer_id) values ($1, $2, $3) RETURNING id", [count, sum, ctx.from.id])
  .catch(e=>{
    console.log(e);
  ...
  const {link: paymentURL, comment, address} =
   await Payments.getTransferInfo(sum, parseInt(orderId));

 ctx.wizard.state.paymentURL = paymentURL

  await ctx.replyWithKeyboard("ORDER_CREATED_TITLE", {name: "submit_payment_keyboard", args: [paymentURL]}, [sum, address, comment, paymentURL])


  ctx.wizard.next();
}})
.addSelect({options: {'1': 'submit_payment'}, cb: async (ctx)=>{
  const {paymentURL, sum: amount, orderId, userObj: {wallet_arrd}} = ctx.wizard.state;

  if (!(await Payments.isOrderPaid(orderId, amount, wallet_arrd))) 
   return ctx.replyWithKeyboard("ORDER_NOT_PAYED", {name: "submit_payment_keyboard", args: [paymentURL]}, [amount])
  ...
  try {
      ...
      await queryRunner.query("update orders set status = 'payed' where customer_id = $1", [ ctx.from.id])
      await queryRunner.query("update users set nft_count = nft_count+$2 where id = $1", [ctx.from.id, ctx.scene.state.count])
