...
const clientScene = new CustomWizardScene("verificationScene").enter(
  async (ctx) => {
    const { edit } = ctx.scene.state;

    if (edit)
      return ctx.replyWithKeyboard(
        ctx.getTitle("ENTER_ADDRESS"),
        "main_menu_back_keyboard"
      );

    await ctx.replyWithKeyboard(ctx.getTitle("ADD_ADDRESS_TITLE"), {
      name: "custom_keyboard",
      args: [["BUTTON_ADD_ADDRESS"], ["add_address"]],
    });
  }
);
clientScene.action("add_address", async (ctx) => {
  await ctx.answerCbQuery().catch((e) => {});

  ctx.replyWithTitle("ENTER_ADDRESS");
});
  ...
  .addStep({
    ...
    cb: async (ctx) => {
      ...
      await ctx.replyWithKeyboard(
        ...
        [amount, toAddr, comment, paymentURL]
      );
